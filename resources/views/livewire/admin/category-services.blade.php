                        ...
                        <div class="card-body ">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                @foreach($services as $service)
                                    <tr>
                                        <td><img src="{{asset('images/services')}}/{{$service->image}}" width="280"height="200"
                                                 alt="{{$service->name}}"></td>
                                        <td>{{$service->name}}</td>
                                        <td> {{$service->slug}}</td>
                                        <td>{{$service->created_at}}</td>
                                        <td>
                                            <a href="{{route('admin.catUpdate',$service->slug)}}"
                                               class="btn btn-success">Update</a>
                                            <a href="javascript:void(0);" wire:click.prevent="delete({{$service->id}})" class="btn btn-danger" onclick="confirm('Are you sure you want to deleted the Category from this group?') || event.stopImmediatePropagation()">Delete</a>
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
